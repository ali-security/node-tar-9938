...
class Unpack extends Parser {
  ...
  [STRIPABSOLUTEPATH] (entry, field) {
    ...
    // strip off the root first, so parts are based on the stripped path
    const [root, stripped] = stripAbsolutePath(p)
    const parts = stripped.replace(/\\/g, '/').split('/')
    if (parts.includes('..') || isWindows && /^[a-z]:\.\.$/i.test(parts[0])) {
      // Reject .. in paths always. For linkpaths, reject .. only for
      // hardlinks (type === 'Link'). Symlinks can use relative paths like
      // '../sibling/file' which are valid if they resolve within the cwd.
    ...
    if (root) {
      ...
      // For symlinks with a stripped root that contain '..', check that the
      // resolved relative path does not escape the extraction directory.
      // Use the validated parts (post-strip), not the raw path.
      if (field === 'linkpath' && type === 'SymbolicLink' &&
          (parts.includes('..') || isWindows && /^[a-z]:\.\.$/i.test(parts[0]))) {
  ...
  [SYMLINK] (entry) {
    const parts = normPath(
      path.relative(this.cwd, path.resolve(path.dirname(entry.absolute), entry.linkpath)),
    ).split('/')
